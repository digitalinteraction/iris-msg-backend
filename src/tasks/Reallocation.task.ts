...
export const MaxDonationAge = parseNumberOrDefault(
  process.env.MAX_DONATION_AGE_IN_MINUTES,
  30 * 60 * 1000
)

export const RellocationInterval = parseNumberOrDefault(
  process.env.DONOR_TICK_IN_MINUTES,
  5 * 60 * 1000
)
...
export class ReallocationTask extends Task<ReallocationContext> {
  ...
  processAttempt(
    ...
    let potentialDonors = organisation.members.filter(
      member =>
        member.isActive &&
  ...
  async sendFcms(
    ...
    await Promise.all(
      donors.map(user => sendNewDonationFcm(messenger, user, i18n, log))
    )
  }

  /** Send twilio fallback sms */
  async sendTwilios(
    attempts: IMessageAttempt[],
    models: IModelSet,
    log: winston.Logger
  ) {
    return Promise.all(
