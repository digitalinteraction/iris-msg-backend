      ...
      files.map(async filename => {
        try {
          ...
          switch (ext) {
            ...
            case 'yml':
              return (this.locales[file] = yaml.safeLoad(data))
            default:
              throw new Error(`Invalid local file ${filename}`)
          }
        } catch (error) {
          console.log(error)
        }
...
export function parseFilename(filename: string): { file: string; ext: string } {
...
export function stringReplace(
  ...
  return input.replace(new RegExp(`\\{${variable}\\}`, 'g'), value)
}
